...
import { AtGuards } from '../Authentication/guards/at.guard';
import { RolesGuard } from '../Authentication/guards/role.guard';
import { Roles } from '../decorators/role.auths';
...
const uploadOptions = {
  fileFilter: (req, file, cb) => {
    if (!file.originalname.match(/\.(pdf|jpg|jpeg|png|gif)$/)) {
      ...
      return cb(
        new HttpException('Only PDF and image files are allowed!', 400),
        false,
      );
    }
    cb(null, true);
